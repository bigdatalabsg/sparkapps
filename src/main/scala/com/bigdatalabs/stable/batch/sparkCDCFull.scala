...
* Author : Anand
* Date : 23-Sep-2024
* Description: Full File Processing with CDC
*/
...
object sparkCDCFull {
    def main(args: Array[String]): Unit = {
        ...
        println(s"6. Prepare for No Change Records")
        ...
        println(s"7. Prepare for Deleted Records")

        val df_delete = df_enriched.filter($"action" === "DELETE").withColumn("is_current", lit(false)).withColumn("is_deleted", lit(true)).withColumn("eff_end_dt", current_date()).select(column_names.map(c => col(c)): _*).cache()

        df_delete.show(false)

        println("8. Merge CLOSED, UPSERT, INSERT , NOACTION Records")

        val df_merged_final = df_noaction.union(df_insert).union(df_close_recs).union(df_upsert).orderBy("sk_id")

        df_merged_final.cache
        df_merged_final.show(false)

        df_merged_final.createOrReplaceTempView("merged_cdc")

        val df_cdc = spark.sql("select * from merged_cdc")
